...
# ==>> abrir cmd e ativar o streamlit: streamlit run main.py
...
def main():
    ...
    minutos_dividir = st.sidebar.selectbox(
        "Dividir o dia em quantos minutosr",
        options=divisores,
    ...
    turnos_possiveis = lista_turnos_possiveis(
                        1, qtd_horas_total, minutos_dividir
    )
    ...
    qtd_turnos = st.sidebar.number_input(
        "Quantidade de turnos possiveis)",
    ...
    grau_polinomio = st.sidebar.number_input(
        "Grau do polinomio para o modelo de previsão",
    ...
    arquivo_ligantes = st.sidebar.file_uploader(
        "Carregar arquivo de ligantes por periodo preenchida",
        type=["xlsx"]
    )
    ...
    if opcao == "Ajustar Polinomio" and \
        ...
        st.pyplot(fig)
    elif opcao == "Rodar Attentus" and botao and arquivo_ligantes and arquivo_duracoes:
